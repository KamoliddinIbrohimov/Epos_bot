...
async def _auto_apply_changes(
    ...
    if holati == "Новый клиент":
        await _auto_new_client(message, parsed, business, business_id, chat_diller_id, token)
        return

    if not business_id:
        return  # без бизнеса больше нечего обновлять

    # Ownership check для остальных статусов.
...
async def _auto_new_client(
    ...
    if not business_id:
        return  # бизнеса нет в Cazad — auto-добавление невозможно
...
async def _auto_fiscal_change(
    ...
    if api_name == new_fiscal:
        await message.reply("ℹ️ Этот фискальный модуль уже внесён в базу.")
        return

    # auth_key оставляем как есть — через _flatten_fk ниже.
    payload = {
        key: _flatten_fk(business.get(key))
        for key in UPDATABLE_FIELDS
        if key in business
    ...
    await message.reply(
        ...
        f"<code>{html.escape(str(new_fiscal))}</code>"
    )
...
async def _auto_address_change(
    ...
    for b in branches:
        ...
        if api_addr and api_addr == new_norm:
            await message.reply("ℹ️ Адрес уже изменён в базе.")
            return
    ...
    await message.reply(
        f"✅ Адрес обновлён: <code>{html.escape(str(new_address))}</code>"
    )
